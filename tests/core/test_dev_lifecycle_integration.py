...
async def lifecycle(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('app.core.attempt_lifecycle.get_session_factory', lambda: sf)
    monkeypatch.setattr('app.core.deploy_trigger.get_session_factory', lambda: sf)
    ...
    async with sf() as s:
        ...
        s.add(Deployment(id='d', codename='AA', scenario_label='demo', project_id='p',
                         target_host_id='h', workspace_path=str(ws), state='pending', current_attempt_id='a'))
        await s.commit()
...
class ControlledRunner:
    ...
    async def start(self, *, private_data_dir, **kwargs):
        self.artifact_dir = private_data_dir
...
async def test_long_run_renews_lock(lifecycle, monkeypatch):
    sf, _, _ = lifecycle
    original_keep = deploy_trigger.keep_attempt_lock
    async def fast_keep(**kwargs):
        return await original_keep(**kwargs, interval_s=.02)
    monkeypatch.setattr(deploy_trigger, 'keep_attempt_lock', fast_keep)
...
async def test_teardown_without_operation_playbook_preserves_workspace(lifecycle):
    ...
    sf, ws, _ = lifecycle
    async with sf() as s:
        (await s.get(Deployment, 'd')).current_attempt_id = None
        await s.commit()
...
async def test_concurrent_submissions_only_start_one_runner(lifecycle, monkeypatch):
    ...
    async with sf() as s:
        dep = await s.get(Deployment, 'd')
        dep.current_attempt_id = None
        await s.delete(await s.get(Attempt, 'a'))
        await s.commit()
...
async def test_generic_attempt_cannot_bypass_operation_guards(lifecycle, scope):
    ...
    async with sf() as s:
        ...
        assert error.value.code == ('TEARDOWN_CONFIRM_MISMATCH' if scope == 'teardown' else 'USE_SCOPED_ENDPOINT')
...
async def test_cancel_during_setup_prevents_late_runner_start(lifecycle, monkeypatch):
    from app.core.attempt_lifecycle import finish_attempt, mark_attempt_running
    from app.core.orphans import track_attempt, untrack_attempt
    from app.routes.v1.deployments.snapshots import cancel_current_attempt
    sf, ws, _ = lifecycle
    monkeypatch.setattr('app.core.runner_detached.signal_running_attempt', AsyncMock(return_value=False))
    task = asyncio.current_task()
    track_attempt('a', task)
    try:
        async with sf() as s:
            response = await cancel_current_attempt('d', s)
            assert response.state == 'pending'
            assert response.sub_reason == 'cancel_requested'
        assert not await mark_attempt_running(attempt_id='a', pid=12345, artifact_dir=ws / 'runner/a')
        assert await finish_attempt(attempt_id='a', rc=None) == 'cancelled'
    finally:
        untrack_attempt('a', task)
