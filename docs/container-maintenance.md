...
An exclusive flock or any nonempty content in the same admission inode rejects new finite requests with HTTP 503 and `Retry-After: 5`, including legacy `/v0` runners and direct `/v1/proxmox` operations. A replaced, public, foreign-owned or otherwise unverifiable gate file also fails closed. Public liveness remains minimal and does not prove that the maintenance gate is valid.
...
2. Attempts the exclusive admission flock and refuses any preexisting intent bytes. Busy finite handlers cause a refusal, allowing the installer to retry after they complete; unfinished intent requires recovery.
...
## Durable installer intent (v2)

`flock-http-intent-v2` adds a persistent fence to the existing flock protocol.
After acquiring its shared flock, every finite request checks that the original
lock inode is empty. Any nonempty bytes, including a partial or unrecognized
marker, return the existing HTTP 503 `MAINTENANCE_ACTIVE`. Authentication still
runs first, and public liveness and the exact read-only event stream remain
available. A marker survives installer exit and API/container restart when the
same persistent inode remains mounted. API startup never truncates it.

A compatible installer holds the original inode exclusively, completes the
idle/provisioning audit, then writes and fsyncs its intent **before** stopping the
old container or starting a fresh candidate. It retains the flock through
migration, validation and record commit. Only that holder may truncate and fsync
its own exact intent after verified candidate readiness plus committed managed
state, or after verified restoration and readiness of the exact prior container.
Closing the descriptor must never clear the marker. Stop/inspect failures,
rollback-readiness failure, or installer death therefore leave requests fenced.
A later helper refuses this state instead of silently taking over the intent.

The container-local helper above does not itself start or clear a durable intent;
it provides the existing held drain/audit primitive. Its caller must implement
the durable cutover sequence. The playbooks container installer validates both
the actual running capability and the immutable candidate image's v2 support;
`flock-http-v1` images do not obey marker bytes and cannot be automatically
upgraded through this v2 running-update path. A disabled gate is also insufficient.

Recovery remains an explicit offline operation. Keep the original lock inode,
private pending record, exact container IDs and backups. Prove failed candidates
stopped before restoring any database; validate the chosen restored container
and its managed bindings before admitting requests. There is no automatic
partial-install recovery endpoint or permission to remove/recreate the gate.
This local filesystem contract does not protect against a privileged external
writer that replaces/truncates the inode, unverified filesystems or data loss.
