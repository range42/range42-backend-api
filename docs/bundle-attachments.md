...
VM bundles can target a recognized managed VM variable. GROUP sources are also supported when every play uses exactly `hosts: "{{ target_group }}"`: the resolution retains `bundle_kind: "GROUP"` and seals `target_kind: "VM"`. The caller cannot supply `target_group`; the compiler and backend bind it to the same declared VM's literal inventory hostname. Names colliding with inventory groups, reserved names and host-pattern expressions are rejected. Existing VM proofs remain compatible.

The GROUP path rejects delegation, local connections/actions, inventory mutation, target selector overrides and externally loaded selector variables in the selected bundle. Ansible connection variable overrides cannot enter caller parameters for either VM or GROUP attachments. Installed role content remains trusted by the operator and covered by the runtime fingerprint; this binding is not an execution sandbox for dependency code. Ansible host patterns can refer to hosts or groups, which is why exact selector binding also checks group-name collisions; see the [official host-pattern documentation](https://docs.ansible.com/projects/ansible/latest/inventory_guide/intro_patterns.html).

Infrastructure, mixed, unknown and broader group scopes are rejected. Playbook imports, sibling bundle lookups and dynamic task/environment/role dependencies are rejected until their complete source dependency closure can be verified. Broad repository-parent environment lookups are also rejected unless their entire reachable directory is an explicit hashed component. Literal task imports inside the selected bundle and installed static role dependencies are supported.

Archive verification resolved all five upstream `firewall/in_vm/os_firewall.baseline.*` GROUP bundles at integration commit `a15086757d39648d5f4c772632c72830da622e05` against an isolated matching runtime and the catalog's `software.configure.firewalls` role. This did not execute guest firewall changes. The role resets existing UFW rules on Ubuntu before applying its baseline, and the UI explains that behavior before attachment.
