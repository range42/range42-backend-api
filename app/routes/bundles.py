...
def _run_create_vms_bundle(req, action_name: str, request_checks_fn) -> JSONResponse:
    ...
    checked_inventory = utils.resolve_inventory(INVENTORY_NAME)
    checked_playbook_init = utils.resolve_bundles_playbook_init_file(action_name, "public_github")
    ...
    for vm_key, item in req.vms.items():
        ...
        rc, events, log_plain, _ = run_playbook_core(
            checked_playbook_init, checked_inventory, tags=vm_key, extravars=extravars,
        )
        if rc != 0:
            payload = {"rc": rc, "log_multiline": log_plain.splitlines()}
    ...
    rc, events, log_plain, _ = run_playbook_core(
        checked_playbook_main, checked_inventory, extravars=extravars,
    )
    payload = {"rc": rc, "log_multiline": log_plain.splitlines()}
    return JSONResponse(payload, status_code=200 if rc == 0 else 500)


def _run_start_stop_bundle(req, action_name: str, proxmox_vm_action: str) -> JSONResponse:
    ...
    rc, events, log_plain, _ = run_playbook_core(
        checked_playbook, checked_inventory, limit=req.proxmox_node, extravars=extravars,
    )
...
def bundles_core_linux_ubuntu_install_docker(req: Request_BundlesCoreLinuxUbuntuInstall_Docker):
    ...
    return _run_bundle_simple(req, "core/linux/ubuntu/install/docker", extravars or None)
...
def bundles_core_linux_ubuntu_install_basic_packages(req: Request_BundlesCoreLinuxUbuntuInstall_BasicPackages):
    ...
    for field, key in [("install_package_basics", "INSTALL_PACKAGES_BASICS"), ("install_package_firewalls", "INSTALL_PACKAGES_FIREWALLS"), ("install_package_docker", "INSTALL_PACKAGES_DOCKER"), ("install_package_docker_compose", "INSTALL_PACKAGES_DOCKER_COMPOSE"), ("install_package_utils_json", "INSTALL_PACKAGES_UTILS_JSON"), ("install_package_utils_network", "INSTALL_PACKAGES_UTILS_NETWORK"), ("install_ntpclient_and_update_time", "INSTALL_PACKAGES_NTP_AND_UPDATE_TIME"), ("packages_cleaning", "SPECIFIC_PACKAGES_CLEANING")]:
        ...
        if val is not None:
            extravars[key] = val
    return _run_bundle_simple(req, "core/linux/ubuntu/install/basic-packages", extravars or None)
...
def bundles_core_linux_ubuntu_install_dotfiles(req: Request_BundlesCoreLinuxUbuntuInstall_DotFiles):
    ...
    return _run_bundle_simple(req, "core/linux/ubuntu/install/dot-files", extravars or None)
...
def bundles_core_linux_ubuntu_configure_add_user(req: Request_BundlesCoreLinuxUbuntuConfigure_AddUser):
    ...
    return _run_bundle_simple(req, "core/linux/ubuntu/configure/add-user", extravars or None)
...
def _check_admin_vms(req):
    ...
    allowed = {"admin-wazuh", "admin-web-api-kong", "admin-web-builder-api", "admin-web-deployer-ui", "admin-web-emp"}
    for r in allowed:
        if r not in req.vms:
    ...
    for vm_name, vm_spec in req.vms.items():
        if vm_spec.vm_id is None:
            raise HTTPException(status_code=400, detail=f"missing key vm_id for {vm_name}")
        if vm_spec.vm_description is None:
            raise HTTPException(status_code=400, detail=f"missing key vm_description for {vm_name}")
        if vm_spec.vm_ip is None:
            raise HTTPException(status_code=400, detail=f"missing key vm_ip for {vm_name}")


def _check_vuln_vms(req):
    if not req.vms or len(req.vms) == 0:
        raise HTTPException(status_code=500, detail="Field vms must not be empty")
    allowed = {"vuln-box-00", "vuln-box-01", "vuln-box-02", "vuln-box-03", "vuln-box-04"}
    for r in allowed:
        if r not in req.vms:
...
for _role in ("admin", "vuln", "student"):
    for _action, _verb in [("start", "vm_start"), ("stop", "vm_stop"), ("pause", "vm_pause"), ("resume", "vm_resume")]:
        ...
        def _make_handler(_an=_action_name, _v=_verb):
            def handler(req: Request_BundlesCoreProxmoxConfigureDefaultVms_StartStopPauseResumeAdminVulnStudentVms):
                return _run_start_stop_bundle(req, _an, _v)
            return handler

        router.add_api_route(
            _path, _make_handler(), methods=["POST"],
...
for _role, _an in _DELETE_ACTIONS.items():
    ...
    router.add_api_route(
        _path, _make_delete_handler(), methods=["DELETE"],
...
for _role, _an in _SNAP_CREATE.items():
    ...
    router.add_api_route(
        _path, _make_snap_create_handler(), methods=["POST"],
...
for _role, _an in _SNAP_REVERT.items():
    ...
    def _make_snap_revert_handler(_an=_an):
        def handler(req: Request_BundlesCoreProxmoxConfigureDefaultVms_RevertSnapshotAdminVulnStudentVms):
            return _run_snapshot_bundle(req, _an, "snapshot_vm_revert")
        return handler

    router.add_api_route(
        _path, _make_snap_revert_handler(), methods=["POST"],
        summary=f"Snapshot {_role} vms ",
        description=f"Snapshot all {_role} virtual machines",
        tags=[_tag],
