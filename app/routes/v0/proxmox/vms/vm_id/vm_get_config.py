...
@router.post(
    path="/vm_get_config",
    summary="Get cdrom configuration of a specific VM -",
    description="This endpoint get the target virtual machine (VM) cdrom configuration details.",
...
def reply_processing(events: list[dict] | list[Any],
                     ...
                     log_plain: str,
                     rc,
                     req: Request_ProxmoxVmsVMID_VmGetConfig) -> dict[str, list | Any]:
...
def request_checks(req: Request_ProxmoxVmsVMID_VmGetConfig) -> dict[Any, Any]:
    """ request checks """

    extravars = {}
    extravars["proxmox_vm_action"] = "vm_get_config"
